...
class FeatureExtractor(nn.Module):
    def __init__(self, sample_obs, vision_type="NatureCNN"):
        super().__init__()
        ...
        if vision_type == "NatureCNN":
            ...
            if self.cam_num == 1:
                fc = nn.Sequential(nn.Linear(n_flatten, feature_size), nn.ReLU())
                extractors["rgb"] = nn.Sequential(resnet, nn.Flatten(), fc)
                self.out_features += feature_size
            else:
                class TwoCamResNet(nn.Module):
                    def __init__(self, resnet, n_flatten, feature_size):
                        super().__init__()
                        self.resnet = resnet
                    ...
                    def forward(self, x):
                        ...
                        env_cam = x[..., :3, :, :]
                        wrist_cam = x[..., -3:, :, :]
                        env_feat = self.resnet(env_cam.float())
                        wrist_feat = self.resnet(wrist_cam.float())
                        combined = torch.cat([env_feat.view(env_feat.size(0), -1), wrist_feat.view(wrist_feat.size(0), -1)], dim=1)
                        return self.fc(combined)
                extractors["rgb"] = TwoCamResNet(resnet, n_flatten, feature_size)
                self.out_features += feature_size
...
class Agent(nn.Module):
    def __init__(self, envs, sample_obs):
        super().__init__()
        self.feature_net = FeatureExtractor(sample_obs=sample_obs, vision_type="ResNet50")
...
class utils_class():
    ...
    class Normalization():
        def __init__(self, shape):
            self.running_ms = utils_class.RunningMeanStd(shape=shape)
    ...
    class RewardScaling:
        def __init__(self, shape, gamma, device="cpu"):
            ...
            self.device = device  # 添加 device 参数，用于指定设备
            self.running_ms = utils_class.RunningMeanStd(shape=self.shape)
            self.R = torch.zeros(self.shape, device=self.device)  # 确保 self.R 在指定设备上

        def __call__(self, x):
            # 将 x 移动到同一设备
            if isinstance(x, torch.Tensor):
                x = x.to(self.device)  # 将 x 移动到 self.R 所在的设备
            else:
                x = torch.tensor(x, dtype=torch.float32, device=self.device)  # 将 x 转换为张量，并移动到正确的设备

            self.R = self.gamma * self.R + x
            self.running_ms.update(self.R)
            x = x / (self.running_ms.std + 1e-8)  # Only divide by std
            return x

        def reset(self):  # When an episode is done, we should reset 'self.R'
            self.R = torch.zeros(self.shape, device=self.device)  # 确保 self.R 在正确的设备上
...
if __name__ == "__main__":
    ...
    _utils.reward_scaling = _utils.RewardScaling(shape=(1,), gamma=0.99)
    ...
    eval_envs = gym.make(args.env_id, num_envs=args.num_eval_envs, reconfiguration_freq=args.eval_reconfiguration_freq, robot_uids=args.robot, **env_kwargs)
    envs = gym.make(args.env_id, num_envs=args.num_envs if not args.evaluate else 1, reconfiguration_freq=args.reconfiguration_freq, robot_uids=args.robot, **env_kwargs)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # rewards[step] = _utils.reward_scaling(reward.view(-1))
    ...
    _utils.reward_scaling.reset()   # reset running reward scaling
